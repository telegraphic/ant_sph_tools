      ...
      for m in range(-n, n + 1):  
        ...
        if len(indx0) > 0:  # special case for theta=0  
           if abs(m) != 1:  
              ...
              Cmn = np.sqrt((2 * n + 1) / 2 * factorial(n - abs(m)) / factorial(n + abs(m)))  
        ...
        for tt in range(len(theta)):  
           ...
           E_ph_mode[:, tt, j-1] = -(1j ** (n + 1)) * (NPdsabsm[tt] * Q[j-1] * np.cos(theta[tt]) - CmnConstant * Q[j-1] * NP[abs(m) + 1, tt])  
        ...
        for tt in range(len(theta)):  
           phi_const = (np.exp(1j * m * phi) * Sign / np.sqrt(n * (n + 1))).T  
           E_th_mode[:, tt, j-1] = 1j ** n * (NPdsabsm[tt] * Q[j-1] * np.cos(theta[tt]) - CmnConstant * Q[j-1] * NP[abs(m) + 1, tt])  
           E_th_mode[:, tt, j-1] = phi_const * E_th_mode[:, tt, j-1]  
